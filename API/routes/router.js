          ...
          if (err) {
            ...
            db.query(
              `INSERT INTO user (idUser, username, password, User_Fname, User_Sname, registered) VALUES ('${uuid.v4()}', ${db.escape(
                ...
                req.body.User_Fname
              )}, ${db.escape(req.body.User_Fname)}, now())`,
...
router.delete("/product/:id", (req, res) => {
  db.query(
    ...
    [req.params.id],
    (error, results, fields) => {
      if (error) res.json({ massage: "Error" });
      else res.json({ massage: "Success" });
    }
  );
});
...
router.get("/orderdetails/:id", (req, res) => {
  db.query(
    "SELECT * FROM ChatBotForSMEsDB.Order WHERE id = ?",
    [req.params.id],
    (error, results, fields) => {
      if (error) res.send({ error: true, message: error });
      res.send({ error: false, data: results });
...
router.get("/orderdetailss/:id", (req, res) => {
  db.query(
    "SELECT O.idOrder_detail, O.fk_order_id, O.fk_product_id, O.od_qty, O.od_total, P.Product_Name, P.Product_Cost FROM Order_detail O INNER JOIN Product P ON O.fk_product_id = P.idProduct WHERE fk_order_id = ?;",
    [req.params.id],
    (error, results, fields) => {
      if (error) res.send({ error: true, message: error });
      res.send({ error: false, data: results });
...
router.post("/update_status/:id", (req, res, next) => {
  ...
  console.log(Order_Status + " /update");

  db.query(
    "UPDATE ChatBotForSMEsDB.Order SET Order_Status = ? WHERE id = ?",
    [Order_Status, id],
    (error, results) => {
      if (error) res.send({ error: true, message: error });
      res.send({ error: false, data: results });
...
router.get("/order", (req, res, next) => {
  db.query(
    "SELECT * FROM ChatBotForSMEsDB.Order ORDER BY id DESC",
    [],
    (error, results, fields) => {
      if (error) res.send({ error: true, message: error });
      res.send({ error: false, data: results });
